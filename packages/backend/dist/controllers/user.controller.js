        ...
        this.deleteAppointment = (req, res) => {
            ...
            user_1.default.findOne({ username: username }, function (err, user) {
                if (err) {
                    ...
                    user_1.default.findOne({ username: app.doctor.username }, (err, doctor) => {
                        if (err) {
                            return res.json({ message: "error" });
                        }
                        user.bookedAppointments = user.bookedAppointments.filter((d) => new Date(d.date).getTime() !== new Date(app.date).getTime());
                        user_1.default.findOneAndUpdate({ username: username }, { $set: { bookedAppointments: user.bookedAppointments } }, (err, resp) => {
                            if (err) {
                                console.log(err);
                            }
                        });
                        doctor.bookedAppointments = doctor.bookedAppointments.filter((d) => new Date(d.date).getTime() !== new Date(app.date).getTime());
                        user_1.default.findOneAndUpdate({ username: doctor.username }, { $set: { bookedAppointments: doctor.bookedAppointments } }, (err, resp) => {
                            if (err) {
                                console.log(err);
                            }
                        });
            ...
            user_1.default.find({ specialization: specialization }, (err, users) => {
                if (err)
                    ...
                    for (let user of users) {
                        user_1.default.findOneAndUpdate({ username: user.username }, { $push: { appointments: appointment } }, (err, user) => {
                            if (err) {
                                return res.json({ message: "error" });
                            }
                            else {
